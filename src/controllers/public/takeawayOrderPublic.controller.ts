...
export class PublicTakeawayOrderController {
    constructor(private publicTakeawayOrderService: PublicTakeawayOrderService) {}
    ...
    bootstrap = catchAsync(async (req: Request, res: Response) => {
        const data = await this.publicTakeawayOrderService.getBootstrapByToken(req.params.token);
    ...
    submit = catchAsync(async (req: Request, res: Response) => {
        ...
        try {
            const data = await this.publicTakeawayOrderService.submitByToken(req.params.token, req.body);
    ...
    getOrder = catchAsync(async (req: Request, res: Response) => {
        const data = await this.publicTakeawayOrderService.resolveOrderByToken(req.params.token, req.params.orderId);
        this.setNoStoreHeaders(res);
        return ApiResponses.ok(res, data);
    });
}
