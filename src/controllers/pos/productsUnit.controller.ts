...
export class ProductsUnitController {
    ...
    findAll = async (req: Request, res: Response) => {
        try {
            const productsUnits = await this.productsUnitService.findAll()
    ...
    findOne = async (req: Request, res: Response) => {
        try {
            const productsUnit = await this.productsUnitService.findOne(req.params.id)
    ...
    findOneByName = async (req: Request, res: Response) => {
        try {
            const productsUnit = await this.productsUnitService.findOneByName(req.params.products_unit_name)
            res.status(200).json(productsUnit)
        } catch (error: any) {
            res.status(500).json({ error: error.message })
        }
