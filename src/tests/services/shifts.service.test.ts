...
describe("ShiftsService payment aggregation", () => {
    ...
    it("sums payment amounts safely", () => {
        expect(
            sumPaymentAmount([{ amount: 10 }, { amount: "20.25" }, { amount: undefined }]),
        ).toBe(30.25);
    });
    ...
    it("sums only cash amounts for drawer expectation", () => {
        expect(
            sumCashPaymentAmount([
                {
                    amount: 100,
                    payment_method: { payment_method_name: "Cash", display_name: "เงินสด" },
                },
                {
                    amount: 250,
                    payment_method: {
                ...
                {
                    amount: "50.5",
                    payment_method: { payment_method_name: "cashbox", display_name: "CASH" },
                },
            ]),
        ).toBe(150.5);
    });
